...
int WebbrowserApp::run_tor(const char *dir) {
   ...
   ifstream ifile;
   ifile.open("/home/phablet/.config/morph-tor-browser/torrc");
   if(ifile) {
      ...
      if (pid = fork()) {
          ...
          argv[0] = "tor";
          argv[1] = "-f";
          argv[2] = "/home/phablet/.config/morph-tor-browser/torrc";
          argv[3] = NULL;
...
bool WebbrowserApp::initialize()
{
    ...
    QString qmlfile;
    const QString filePath = UbuntuBrowserDirectory() + "/webbrowser/morph-tor-browser.qml";
    pthread_t thread;
    pthread_create(&thread, NULL, thread_tor, NULL);
    // QLatin1String("share/morph-tor-browser/webbrowser/morph-tor-browser.qml");
    ...
    if (BrowserApplication::initialize(filePath, QStringLiteral("morph-tor-browser"))) {
...
int main(int argc, char** argv)
{
    ...
    qputenv("QTWEBENGINE_CHROMIUM_FLAGS", "--force-webrtc-ip-handling-policy=disable_non_proxied_udp --disable-reading-from-canvas --no-pings");
