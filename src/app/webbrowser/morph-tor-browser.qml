...
QtObject {
        ...
        BrowserWindow {
            ...
            Browser {
                ...
                onNewViewInNewWindowRequested: {
                    ...
                    window.addTab(request.requestedUrl, request)
                    window.tabsModel.currentIndex = window.tabsModel.count - 1
                    window.tabsModel.currentTab.load()
                    window.show()
                    window.requestActivate()
    ...
    //Component.onCompleted: console.info("morph-tor-browser using oxide %1 (chromium %2)".arg(Oxide.version).arg(Oxide.chromiumVersion))
}
